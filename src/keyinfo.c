...
static const struct algo_table_s enc_algo_table[] = {
  ...
  { /* iso.member-body.ru.rans.cryptopro.gostR3410-2001 */
   "1.2.643.2.2.19",
   "\x2a\x85\x03\x02\x02\x13", 6,
   1, PKALGO_GOST, "gost", "*--km-b--p-C?D-qu",
   "\x30\x30\x04\x04\xa0\x06\xa0\x30\x06\x30\x06\x06\x03\x04\x04",
   "-C?D", "\x30\x06\x06" },
      ...
      if (pkalgo == PKALGO_GOST && *parmdesc == 'q' && !parmdesc[1])
        {
          for (i=0; i < parmidx; i++)
            if (parm[i].namelen == 1 && parm[i].name[0] == 'q')
              {
                const unsigned char *p = parm[i].value;
                size_t l = parm[i].valuelen;
                if (l && p[0] == 0x04)
                  {
                    p++; l--;
                  }
                unsigned char *tmp = xtrymalloc (l);
                if (!tmp)
                  {
                    err = gpg_error_from_syserror (); 
                    goto leave;
                  }
                _ksba_flip_ecc_key (p, l, tmp);
                _ksba_der_add_bts (dbld, tmp, l, 0);
                xfree (tmp);
                break;
              }
        }
      else if (*parmdesc == 'q' && !parmdesc[1])
        {
      ...
      for (; *elem; ctrl++, elem++)
        {
          ...
          if (is_int && *elem != '-')
            { 
              ...
              tmp[0] = *elem; tmp[1] = 0;
              if (reverse_all && algo_table[algoidx].pkalgo == PKALGO_GOST
                  && *elem == 'S')
                {
                  put_stringbuf (&sb, "(1:r");
                  put_stringbuf_mem_sexp_rs (&sb, der + len/2, len/2);
                  put_stringbuf (&sb, ")");
                  put_stringbuf (&sb, "(1:s");
                  put_stringbuf_mem_sexp_rs (&sb, der, len/2);
                  put_stringbuf (&sb, ")");
                  der += len;
                  derlen -= len;
                }
...
/* Take a sig-val style S-expression and convert it into a DER encoded
 * signature.  Only the GOST variant, which requires byte reversal of
 * R and S, is currently supported.  */
static gpg_error_t
cryptval_from_sexp (ksba_const_sexp_t sexp,
                    unsigned char **r_der, size_t *r_derlen)
{
  gpg_error_t err = 0;
  ...
  const char *algo_oid;
  pkalgo_t pkalgo;
  struct { const unsigned char *value; size_t len; } r = {NULL,0},
                                                     sigs = {NULL,0};
  ksba_der_t dbld = NULL;
  ...
  *r_derlen = 0;
  ...
  if (!(n = snext (&s)))
    return gpg_error (GPG_ERR_INV_SEXP);
  algo_oid = oid_from_buffer (s, n, &pkalgo, 1);
  if (!algo_oid)
    return gpg_error (GPG_ERR_UNSUPPORTED_ALGORITHM);
  s += n;
  ...
  _ksba_der_release (dbld2);
      ...
      if (*s != ')') { err = gpg_error (GPG_ERR_INV_SEXP); goto leave; }
      s++;
    }
  if (*s != ')') { err = gpg_error (GPG_ERR_INV_SEXP); goto leave; }
  s++;

  if (pkalgo != PKALGO_GOST || !r.value || !sigs.value)
    { err = gpg_error (GPG_ERR_UNSUPPORTED_ALGORITHM); goto leave; }
...
gpg_error_t
_ksba_sigval_from_sexp (ksba_const_sexp_t sexp,
                        unsigned char **r_der, size_t *r_derlen)
{
  return cryptval_from_sexp (sexp, r_der, r_derlen);
}
